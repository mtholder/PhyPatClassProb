...
Exploiting the fact that distinct combinations that give the same result allows the answer to be calculated with fewer operations. 
We are can sketch out a rough computational complexity for a symmetric version of the algorithm.
...
Figure \ref{symmetryFig} shows a graph representing the calculations required for 
...
The time savings for the symmetric model expand for larger values of $k$.
Implementing a symmetric model specialization will enable considerations of characters with many more states.
...
\item We will adapt our testing program to perform the model adequacy approach of Waddell {\em et al.~}\citep{WaddellOP2009}. This will entail the calculation of goodness-of-fit statistics from the probabilities that our algorithms compute. Our implementation will provide the community of evolutionary biologist with a tool for assessing model adequacy on large phylogenetic trees.
...
The authors would like to thank NSF and the Initiative for Maximizing Student Diversity for funding.
} 
